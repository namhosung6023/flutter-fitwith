...
class _MemberChecklistState extends State<MemberChecklist> {
  ...
  void _getdata() async {
    ...
    Response response = await dio.get(
        'http://10.0.2.2:3000/premium/checklist/user/$userId');
    setState(() {
      ...
      for(int i = 0; i < workoutList.length; i++) {
        Checklist workout = Checklist(workoutList[i]['name'], workoutList[i]['contents'], checkListId, workoutList[i]['_id'], workoutList[i]['isEditable']);
  ...
  void _putCheckboxData(bool isChecked, Checklist checklist) async {
    ...
    try {
      Response response = await dio.put('http://10.0.2.2:3000/premium/checklist/user/checkbox/$userId', data: stringData);
      print(response);
    } catch(err) {
      print('error: $err');
    }
  ...
  Widget build(BuildContext context) {
    if (this._member.checklist.length == 0) this._member.checklist.addAll(checkList);
    return ListView(
  ...
  Widget _buildChecklistItem(checklist) {
    final color = (checklist.isEditable)
        ...
        Row(
          ...
          children: [
            ...
            Padding(
              ...
              child: Image.asset(
                  'assets/ic_dumbbell.png', width: 32.0, fit: BoxFit.cover),
            ),
